...
public class Robot extends SampleRobot {
	...
	Talon lift = new Talon(0); //initializing lift talon
	...
	public void updateStickValues(){ // Update stick values function to be called inside for the main WHILE loop
		...
		if (getTrueY() > 0 && getTurningTrueY() > 0 && otherPosTRUE == 0) {xTank = 1 / sanicMode(); otherPosTRUE = 1;}
		if (getTrueY() < 0 && getTurningTrueY() < 0 && otherPosTRUE == 0) {xTank = -1 / sanicMode(); otherPosTRUE = 1;}
		// If joysticks are in x+ and x- turn in respective direction 
		if (getTrueY() < 0 && getTurningTrueY() > 0 && otherPosTRUE == 0) {yTank = 1 / sanicMode(); otherPosTRUE = 1;}
		if (getTrueY() > 0 && getTurningTrueY() < 0 && otherPosTRUE == 0) {yTank = -1 / sanicMode(); otherPosTRUE = 1;}
		//Straif based on joystick y axis
		if (getTrueX() > 0 && getTurningTrueX() > 0) {zTank = 1 / sanicMode();}
		if (getTrueX() < 0 && getTurningTrueX() < 0) {zTank = -1 / sanicMode();}
